...
export function NoteEditor() {
  ...
  // Set initial position when first opened
  useEffect(() => {
    if (openNoteId && pos === null) {
  ...
  // Sync local title from note metadata
  useEffect(() => {
  ...
  if (!openNoteId || !mounted || pos === null) return null;
  ...
  const panel = (
    ...
    >
      ...
      <div
        className="shrink-0 flex items-center gap-2 px-2 py-2 bg-stone-50 border-b border-stone-200 cursor-grab active:cursor-grabbing touch-none"
        onPointerDown={onDragStart}
        onPointerMove={onDragMove}
        onPointerUp={onDragEnd}
      >
        <GripVerticalIcon className="w-3.5 h-3.5 text-stone-300 shrink-0" />
