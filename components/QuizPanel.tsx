...
export function QuizPanel({ questions }: Props) {
  ...
  return (
    <div className="rounded-xl border border-[#DEDAD0] dark:border-white/[0.06] bg-white dark:bg-white/[0.04] overflow-hidden" dir="rtl">
      ...
      >
        <div className="flex items-center gap-2">
          <svg className="w-4 h-4 text-[#F0BC53] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            <circle cx="12" cy="12" r="10" />
            <path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3" />
            <path d="M12 17h.01" />
          </svg>
          <span className="text-sm font-bold text-[#0F2822] dark:text-white/80">اختبر فهمك</span>
          <span className="text-xs text-[#0F2822]/40 dark:text-white/30">{questions.length} {questions.length === 1 ? "سؤال" : "أسئلة"}</span>
          {submitted && (
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${scoreBannerStyle}`}>
              {score}/{questions.length}
            </span>
          )}
        </div>
      ...
      {open && (
        <div className="border-t border-[#DEDAD0] dark:border-white/[0.06]">
          ...
          {submitted && (
            <div className={`flex items-center gap-3 mx-4 mt-4 rounded-lg border px-4 py-3 text-sm font-semibold ${scoreBannerStyle}`}>
              <span className="text-base">{score === questions.length ? "🎉" : score / questions.length >= 0.6 ? "✅" : "📖"}</span>
              <span>أجبت بشكل صحيح على {score} من أصل {questions.length} {questions.length === 1 ? "سؤال" : "أسئلة"}</span>
            </div>
          )}
          ...
          <div className="p-4 flex flex-col gap-4">
            ...
            {questions.length > 1 && (
              <div className="flex items-center justify-center gap-1.5">
                {questions.map((_, i) => {
                  ...
                  const correct = submitted && selected[i] === questions[i].correct;
                  const wrong = submitted && selected[i] !== null && selected[i] !== questions[i].correct;
                  return (
                    <button
                      key={i}
            ...
            <p className="text-sm font-semibold text-[#0F2822]/90 dark:text-white/80 leading-relaxed">
              <span className="text-[#F0BC53] ml-1">{current + 1}.</span> {q.text}
            </p>

            {/* Options */}
            <div className="flex flex-col gap-2">
              {q.options.map((opt, oi) => {
                let style = "border-[#DEDAD0] dark:border-white/[0.08] text-[#0F2822]/70 dark:text-white/60 hover:border-[#F0BC53]/50 hover:bg-[#F0BC53]/5 cursor-pointer";

                if (submitted) {
                  if (oi === q.correct)
                    style = "border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-600 text-emerald-700 dark:text-emerald-400 cursor-default";
                  else if (oi === userPick)
                    style = "border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-600 text-red-600 dark:text-red-400 cursor-default";
                  else
                    style = "border-[#DEDAD0]/50 dark:border-white/[0.04] text-[#0F2822]/30 dark:text-white/25 cursor-default";
                } else if (userPick === oi) {
                  style = "border-[#F0BC53] bg-[#F0BC53]/10 text-[#0F2822] dark:text-white font-medium cursor-pointer";
                }
                ...
                return (
                  ...
                  >
                    {submitted && oi === q.correct && (
                      <svg className="w-3.5 h-3.5 shrink-0 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M20 6L9 17l-5-5" /></svg>
                    )}
                    {submitted && oi === userPick && oi !== q.correct && (
                      <svg className="w-3.5 h-3.5 shrink-0 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M18 6L6 18M6 6l12 12" /></svg>
                    )}
                    {(!submitted || (oi !== q.correct && oi !== userPick)) && (
                      <span className={`w-3.5 h-3.5 shrink-0 rounded-full border-2 flex-shrink-0 ${userPick === oi && !submitted ? "border-[#F0BC53] bg-[#F0BC53]" : "border-current opacity-30"}`} />
                    )}
                    <span>{opt}</span>
                  </button>
                );
